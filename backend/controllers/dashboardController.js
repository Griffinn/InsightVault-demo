...
export const getDashboardSummary = async (req, res) => {
    try {
        const db = await getDBConnection(); // Now a Turso client
        ...
        // --- 1. Total Balance ---
        const totalBalanceQuery = `SELECT SUM(amount) AS totalBalance FROM transactions`;
        // CHANGE: .execute() and access .rows[0]
        const totalBalanceResult = await db.execute(totalBalanceQuery);
        const totalBalance = totalBalanceResult.rows[0]?.totalBalance || 0;
        ...
        const currentMonthlyMetricsQuery = `
            ...
            GROUP BY type
        `;
        // CHANGE: Object with sql and args
        const currentMetricsResult = await db.execute({
            sql: currentMonthlyMetricsQuery,
            args: [currentMonthString]
        });
        const currentMetrics = currentMetricsResult.rows;
        ...
        // --- 3. Monthly Metrics (Previous Month) ---
        const previousMonthlyMetricsQuery = `
            ...
            AND type = 'income'
        `;
        const previousIncomeResult = await db.execute({
            sql: previousMonthlyMetricsQuery,
            args: [previousMonthString]
        });
        const previousMonthlyIncome = previousIncomeResult.rows[0]?.total || 0;
        ...
        res.json({
            ...
            monthlySavings: parseFloat(monthlySavings).toFixed(2),
            totalBalanceComparison: staticComparison,
            monthlyIncomeComparison: incomeComparison,
            monthlyExpenseComparison: staticComparison,
            monthlySavingsComparison: staticComparison,
        });
